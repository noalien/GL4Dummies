...
void _abscisses(vertex_t * p0, vertex_t * p1, vertex_t * absc, int replace) {
  ...
  if(u > v) { // 1er octan
	...
	pp1 = sqrt(x * x + y * y) / d;
	absc[k].color[0] = (1.0f - pp1) * p0->color[0] + pp1 * p1->color[0];
	absc[k].color[1] = (1.0f - pp1) * p0->color[1] + pp1 * p1->color[1];
	absc[k].color[2] = (1.0f - pp1) * p0->color[2] + pp1 * p1->color[2];
	absc[k].color[3] = (1.0f - pp1) * p0->color[3] + pp1 * p1->color[3];
	absc[k].texCoord[0] = (1.0f - pp1) * p0->texCoord[0] + pp1 * p1->texCoord[0];
	absc[k].texCoord[1] = (1.0f - pp1) * p0->texCoord[1] + pp1 * p1->texCoord[1];
	if(delta < 0) {
	  ++k;
	  y += pasY;
	  delta += incO;
