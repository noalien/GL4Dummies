...
#ifdef __GLES4D__
/* globales utilisées dans gl4dummies.h dans le cas de GLES3.0 - epsilon */
_es30_tex_info_t *_tex_info = NULL;
GLint _tex_info_size = 0;
#endif
...
static double getElapsedTime_sub2(void)
{
  ...
  diff = (t.tv_sec - t0.tv_sec) * 1000000 + (t.tv_usec - t0.tv_usec);
  return diff / 1000.;
}
...
char *pathOf(const char *path)
{
  int spos = -1;
  char *tmp, *ptr;
  tmp = malloc((strlen(path) + 1) * sizeof *tmp);
  assert(tmp);
  strcpy(tmp, path); //strdup(path);
  ptr = tmp;
  while (*ptr)
  {
    if (*ptr == '/' || *ptr == '\\')
