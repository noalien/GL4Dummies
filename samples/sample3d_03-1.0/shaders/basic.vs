...
float height(vec2 p) {
  ...
  float amp = 0.2 * clamp(1.0 - length(p), 0, 1);
  return amp * sin(-freq * length(p) + dephase);
}
...
void main(void) {
  ...
  vsoModPosition = modelViewMatrix * vec4(pos, 1.0);
  gl_Position = projectionMatrix * vsoModPosition;
}
