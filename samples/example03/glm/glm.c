...
/*
  Ajout par Vincent Boyer
  Eviter qu'a chaque frame, on est un affichage d'erreur
*/
#define DEBUG if(0)

GLvoid
__glmWarningDraw(char *s){

  DEBUG __glmWarning(s);
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    /* ajoutée par FB */
    GLint cp = 0; glGetIntegerv(GL_CURRENT_PROGRAM, &cp);

    assert(model);
    assert(model->vertices);

    //glmDisplayInfo(model);

    /* do a bit of warning */
    if (mode & GLM_FLAT && !model->facetnorms) {
        __glmWarningDraw("glmDraw() warning: flat render mode requested "
		     "with no facet normals defined.");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        __glmWarningDraw("glmDraw() warning: smooth render mode requested "
		     "with no normals defined.");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        __glmWarningDraw("glmDraw() warning: texture render mode requested "
		     "with no texture coordinates defined.");
        mode &= ~GLM_TEXTURE;
    }
    if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
    ...
    if (mode & GLM_COLOR && !model->materials) {
        __glmWarningDraw("glmDraw() warning: color render mode requested "
		     "with no materials defined.");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        __glmWarningDraw("glmDraw() warning: material render mode requested "
		     "with no materials defined.");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        __glmWarningDraw("glmDraw() warning: color and material render mode requested "
		...
		if(blending == blenditer) {
			...
			if (mode & GLM_TEXTURE) {
				...
				if(map_diffuse == -1) {
				    ...
				    /* ajoutée par FB */
				    glUniform1i(glGetUniformLocation(cp, "useTexture"), 0);
				} else {
				    glBindTexture(_glmTextureTarget, model->textures[map_diffuse].id);
				    /* ajoutée par FB */
				    glUniform1i(glGetUniformLocation(cp, "useTexture"), 1);
 				}
				glBegin(GL_TRIANGLES);
			    }
			}
