 ...
 */
// static float bezierdegre2simple(float depart, float middle, float arrivee, float t);
...
static void diamond_square(int taille, const char* filename, int choice, int nb_args, char** args);
static void diamond_square_algo(int taille, const char* filename, int choice, int nb_args, char** args, float deplacement_diamond, float deplacement_square);
static void sharpness_recalculation(float ** point_tab, int taille, float min_height, float max_height);
...
// 			point_tab[x_it][y_it] += (bezierdegre2simple(courbes[y_it].a, courbes[y_it].b, courbes[y_it].c, modfloat((float)x_it / (float)taille * (float)nb_motifs, 1)/*(float)x_it / taille)*/) + sin((float)x_it / taille)) / 3;
// 		}
// 	}
...
// 			point_tab[x_it][y_it] += (bezierdegre2simple(courbes[x_it].a, courbes[x_it].b, courbes[x_it].c, modfloat((float)y_it / taille * nb_motifs, 1)) + sin((float)y_it / taille)) / 3;
...
static void diamond_square(int taille, const char* filename, int choice, int nb_args, char** args) {
	...
	fprintf(stdout, "génération de la heightmap\nla taille sera arrondie à la taille 2^n+1 supérieure\n");
...
static void diamond_square_algo(int taille, const char* filename, int choice, int nb_args, char** args, float deplacement_diamond, float deplacement_square) {
	...
	int pas = taille;
	//le nb de cases remplies a la derniere iteration, on declare pour que ce soit hors du stack
	int it;
	int it2;
	int x_it;
	int y_it;
	...
	printf("%f  %f  %f  %f\n", point_tab[0][0], point_tab[0][taille - 1], point_tab[taille - 1][0], point_tab[taille - 1][taille - 1]);
		...
		for (y_it = pas; y_it < taille - 1; y_it += pas << 1) {
			for (x_it = pas; x_it < taille - 1; x_it += pas << 1) {
				...
				if (point_tab[x_it][y_it] == 0) {
					...
					point_tab[x_it][y_it] = value_simple(parenting_height, deplacement_diamond);
		...
		for (y_it = 0; y_it < taille; y_it += pas) {
			for (x_it = 0; x_it < taille; x_it += pas) {
				...
				if (point_tab[x_it][y_it] == 0) {
					...
					point_tab[x_it][y_it] = value_simple(parenting_height, deplacement_square);
	...
	for (y_it = 0; y_it < taille; ++y_it) {
		for (x_it = 0; x_it < taille; ++x_it) {
			...
			if (nb_voisins > 0) {
				...
				voisins /= nb_voisins;
				// c est degueulasse
				diff_tab[x_it][y_it] = voisins - (((voisins - point_tab[x_it][y_it]) / ((max_height - min_height)/* / 2*/)) - (((voisins - point_tab[x_it][y_it]) / ((max_height - min_height) /* / 2*/)) - 1));
			}
			else
