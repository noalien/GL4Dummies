...
/* on créé une variable pour stocker les identifiants des programmes GPUs */
static GLuint _pId[2] = { 0 };
/* choix entre les X shaders */
static int _program_idx = 0;
...
void init(void) {
  ...
  _pId[0] = gl4duCreateProgram("<vs>shaders/v1.vs", "<fs>shaders/v1.fs", NULL);
  _pId[1] = gl4duCreateProgram("<vs>shaders/v2.vs", "<fs>shaders/v2.fs", NULL);
...
void keyd(int keycode) {
  if(keycode == 'p') {
    _program_idx = (_program_idx + 1) % (sizeof _pId / sizeof *_pId);
  }
}
...
void draw(void) {
  ...
  /* utiliser le programme GPU "_pId[_program_idx]" */
  glUseProgram(_pId[_program_idx]);
