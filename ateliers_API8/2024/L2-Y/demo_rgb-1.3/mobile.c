...
static const vec3_t _g = {0.0f, -9.8f / 2.0f, 0.0f};
...
void mobile_pulse(float intensite_son) {
  int i;
  /* cuisine ... */
  intensite_son = pow(intensite_son, 2.0f) / 2.7f;
  for(i = 0; i < _nb_mobiles; ++i) {
    _mobiles[i].a.y = _g.y + intensite_son / 2.0f;
    _mobiles[i].v.y += intensite_son;
  }
}
...
void mobile_simu(void) {
  ...
  if(dt > 0.2f) dt = 1.0f / 60.0f;// petit hack pour ré-init les
                                  // calculs si le temps passé entre
                                  // deux frames est trop log. Arrive
                                  // quand la simu est dans une démo,
                                  // et qu'elle a besoin de ré-init au
                                  // bon moment.
  t0 = t;
  for(i = 0; i < _nb_mobiles; ++i) {
    int collision_mur = 0;
    ...
    if(collision_mur) {
      ...
      const float pc = 0.86f;
      GLfloat n = (_mobiles[i].v.x * _mobiles[i].v.x +
                   _mobiles[i].v.y * _mobiles[i].v.y +
                   _mobiles[i].v.z * _mobiles[i].v.z);
      if(sqrt(n) > 1.0f) {
        _mobiles[i].v.x *= pc;
