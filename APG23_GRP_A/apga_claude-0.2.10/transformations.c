...
static inline int _back_face(triangle_t * t);
...
void claude_draw(surface_t * sp, const int * viewport) {
  for(int i = 0; i < sp->n; ++i) {
    ...
    if((sp->triangles[i].v[0].d < 0.0f || sp->triangles[i].v[1].d < 0.0f || sp->triangles[i].v[2].d < 0.0f) ||
      ...
      continue;
    if(claude_is_enabled(CL_OPT_BACK_FACE_CULLING) && _back_face(&(sp->triangles[i])))
      continue;
...
int _back_face(triangle_t * t) {
  vec3 a = { t->v[1].x - t->v[0].x, t->v[1].y - t->v[0].y, t->v[1].z - t->v[0].z };
  vec3 b = { t->v[2].x - t->v[1].x, t->v[2].y - t->v[1].y, t->v[2].z - t->v[1].z };
  vec3 c;
  vec3cross(c, a, b);
  if(c[2] < 0.0f)
    return 1;
