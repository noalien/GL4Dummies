...
void colorize_surface(surface_t * s, vec4 color) {
  for(int i = 0; i < s->n; ++i)
    for(int j = 0; j < 3; ++j) {
      s->triangles[i].v[j].r = color[0];
      s->triangles[i].v[j].g = color[1];
      s->triangles[i].v[j].b = color[2];
      s->triangles[i].v[j].a = color[3];
    }
}
