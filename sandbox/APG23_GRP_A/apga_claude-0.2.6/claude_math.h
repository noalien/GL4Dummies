...
extern "C" {
  ...
  static inline void vec3cross(vec3 r, vec3 u, vec3 v) {
    r[0] = u[1] * v[2] - u[2] * v[1];
    r[1] = u[2] * v[0] - u[0] * v[2];
    r[2] = u[0] * v[1] - u[1] * v[0];
  }
  ...
  static inline float vec3dot(vec3 u, vec3 v) {
  ...
  static inline void mat4inverse(mat4 m) {
    mat4 tmp;
  ...
  static inline void mat4transpose(float * m) {
    float t;
    t = m[1];  m[1]  = m[4];  m[4]  = t;
    t = m[2];  m[2]  = m[8];  m[8]  = t;
  ...
  static inline void lookAt(mat4 m, float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ) {
