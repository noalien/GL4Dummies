...
void _hline(vertex_t * aG, vertex_t * aD) {
  ...
  for(int x = x0, yw = y * w; x <= x1; ++x) {
    ...
    depths[yw + x] = depth;
    /* couleurs */
    r  = paG * aG->color[0] + paD * aD->color[0];
    g  = paG * aG->color[1] + paD * aD->color[1];
    b  = paG * aG->color[2] + paD * aD->color[2];
    a  = paG * aG->color[3] + paD * aD->color[3];
